...
### Visual cut: the decisions on the page

`DECISIONS_VISUAL=1` renders the race over the page navvi was reading. In capture mode the script watches navvi's own crawler page through `CrawlDeps.onPage`, takes a viewport screenshot just before each batch is asked, and measures the element each option names from the `optionContext` navvi attached to it: a control by ARIA role and name (its href if that fails), a result group as the union of `container > item`, a field candidate as the first element on its path whose text or attribute begins with sample 1's value, a list candidate as the union of the elements holding sample 1's values, a link by href and text. These reads are read-only and untimed; they are saved as `capture/visual.json` and `capture/visual/batch-N.png`. Jev's pick is outlined green (inner box), the other lane's purple (outer box), each appearing at the measured time that lane answered; `≠` marks a pick that differs from the reference. Publishing writes `docs/decisions-race-visual.*`, or `docs/decisions-race-claude-code-visual.*` with `DECISIONS_VS=claude-code`.

    set -a; . /path/to/.env; set +a
    DECISIONS_VISUAL=1 DECISIONS_OUT=/tmp/navvi-decisions npx tsx scripts/record-decisions.ts
    DECISIONS_MODE=race DECISIONS_VISUAL=1 DECISIONS_VS=claude-code \
      DECISIONS_CAPTURE=/tmp/navvi-decisions/navvi-decisions-XXXX DECISIONS_OUT=/tmp/navvi-decisions npx tsx scripts/record-decisions.ts
    DECISIONS_MODE=render DECISIONS_VISUAL=1 DECISIONS_PUBLISH=1 \
      DECISIONS_SOURCE=/tmp/navvi-decisions/navvi-decisions-YYYY npx tsx scripts/record-decisions.ts

September 24, one capture of 20 decisions in 8 batches (it adds the `list.points` question): over the AI Gateway, Haiku 13.9 / 12.2 / 13.7 s and Jev 3.6 / 2.7 / 3.8 s, 3.8× in the median run (3.6–4.6×); through Claude Code on `685aa9f`, Haiku 43.1 / 38.2 / 38.4 s and Jev 3.7 / 2.8 / 4.0 s, 11.8× in the median run (9.6–13.9×). Jev matched the reference 18/20 in every run: both misses name the Search Hacker News logo as the click target on a step whose action was DONE, where the target is unused. Haiku matched 20/20 over the Gateway; through Claude Code 19/20 or 20/20, once answering `list.points` as a list. A first capture was discarded after Haiku over the Gateway answered outside the offered indices on `list.points` in one run.
