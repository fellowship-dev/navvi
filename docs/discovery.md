...
Status: **design, not built.** Written 2026-09-22 after a client engagement
exposed that navvi can replay a declared source but cannot find one.
...
Three hand-authored scrapers for a client's e-commerce sites are what that
produces: three sites, three compiled scrapers, 27–44% product coverage:

```
Store A   listPrice  :scope > body.modal-open > … > span.value:nth-of-type(1)
Store B   stock      :scope > body.one-col.christmas-pattern > … > div.desc:nth-of-type(2)
Store C   listPrice  p.font-semibold.leading-16.leading-22
```

A modal-state class, a seasonal class, and a Tailwind line-height. Meanwhile
Store B states both prices in its own `<meta>` tags, Store A states name, SKU,
brand, price and availability in JSON-LD, and Store C's page fetches its
prices as JSON from its own API.
...
`listPrice`. Three prompts failed to make a model pick Store C's list price
...
1. **Flatten.** Every leaf of every captured payload and declared block becomes
   a `(path, value)` pair — `productData.prices[price-list] = 1990`.
...
cascade run bound Store B's `productName` to a JSON-LD `name` that was the
store's own name on every page — the Organization node, not the Product. A
variation check kills that automatically, without knowing anything about
schema.org. It is the same rule the client's value invariants apply after a
compile, moved to where it prevents the defect instead of detecting it.
...
price: `price-list` 1990, `price-sale` 1791?" — rather than to search a
...
{ "source": "network", "match": "products/detail",
  "path": "productData.prices[price-list]", "fingerprint": { … } }
...
When tier 3 does run, it must refuse what it produced for the client scrapers
above. The audit already exists (`diagnose.js`, to be moved here) and scores:
absolute paths from `body`, positional `:nth-of-type` chains, transient state
classes (`modal-open`), campaign classes (`christmas-pattern`), and selectors
made only of styling utilities. A rotten selector is a recompile, not a commit.

## Healing promotes

Store A's compiled scraper carried the correct selector all along — healing had
...
```
$ navvi probe https://example-store.test/products/…
...
1. Move the general code out of the client's repo: the declared-data
   extractor, the struck-price rule, the selector audit, the value
   invariants, the admission loop. It is navvi's product sitting in a
   client's repository.
...
Bot protection is orthogonal and real: Store C sits behind a bot-detection
vendor and that cost does not move. A site with no declared data, no API and
hostile markup still needs tier 3 and still deserves a low confidence score —
the probe should say so rather than promise a scraper.
